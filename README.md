...
<h1 align="center">🐱 Desktop Pet 桌面宠物</h1>
...
前往 [Releases](../../releases) 页面下载最新的 `桌面宠物.exe`，双击即可运行，无需安装。
...
#### 🎨 绿幕去背算法
宠物素材使用纯绿色（#00FF00）背景，运行时通过 Canvas 逐像素分析 RGB 通道，自动去除绿色背景并平滑边缘，实现完美透明。

#### 🖱️ 鼠标穿透方案
采用 CSS `pointer-events: none`（全局穿透）+ `pointer-events: auto`（宠物区域可交互）的方案，而非 Electron 的 `setIgnoreMouseEvents` API。后者在 Windows 上会触发 DWM 重绘标题栏。

#### 🛡️ 多分辨率适配
使用 `screen.getPrimaryDisplay().workArea` 获取完整工作区坐标（含任务栏偏移），确保在任何分辨率和任务栏位置下都能正确定位。每 5 秒进行一次安全边界检查。
